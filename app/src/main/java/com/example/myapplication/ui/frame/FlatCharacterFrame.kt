...
class FlatCharacterFrame(
    ...
    private var mTranslationHandle = 0
    ...
    companion object {
        private const val VERTEX_SHADER_CODE = "attribute vec3 aVertexPosition;" +
                ...
                "gl_Position = mvPosition + vec4(uTranslation, 1.0);" +
    ...
    init {
        ...
        mTextureImageHandle =
            GraphicUtils.loadOpenGLTextureFromResources(resources, R.drawable.saitama)
        ...
        mTranslationHandle = GLES32.glGetUniformLocation(mProgram, "uTranslation")
        renderer.checkGlError("glGetUniformLocation")

        GLES32.glEnableVertexAttribArray(mTextureCoordinatesHandle)
    }

    fun draw(mvpMatrix: FloatArray, translationModel: TranslationModel?) {
        GLES32.glUseProgram(mProgram) // Add program to OpenGL environment
        ...
        translationModel?.let { translationModelSafe ->
            translation[0] = translationModelSafe.translationX
            translation[1] = translationModelSafe.translationY
            translation[2] = translationModelSafe.translationZ
        }
        GLES32.glUniform3fv(mTranslationHandle, 1, translation, 0)
